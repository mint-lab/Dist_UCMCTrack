    ...
    with open(det_result_file) as f_in:
        ...
        for det_result in det_results:
            ...
            if frame_id == '1':
                ...
                # Distort points 
                pts = generate_all_midpoints(u, v, w, h, num_points=10)  
                # for pt in pts:
...
def create_distorted_mot20(gt_file, save_file, cam_dist, cam_type):
...
if __name__ == "__main__":
    ...
    # KB best
    dist_coeffs = [ np.array([-0.25, -0.05, 0.0, 0.0]),  # 01
                   np.array([-0.25, -0.05, 0.0, 0.0]),   # 02
                   np.array([-0.55, 0.15, 0.0, 0.0]),   # 03
                   np.array([-0.55, 0.15, 0.0, 0.0])]   # 05

    
    for seq, dist_coeff  in zip(sequences, dist_coeffs):    

        # Get Intrinsic Matrix from CamParafile 
        K,_,_ = readCamParaFile(f"cam_para/MOT20/{seq}.txt")
        ...
        # src_img = f"dataset/MOT20/train/{seq}/img1/000001.jpg"
        # det_file = f"det_results/mot20/{seq}.txt"
        # test_distort_new(src_img, f, c, dist_coeff, det_file, cam_type)
        
        # Create distorted detection result 
        src_det = f"det_results/mot20/{seq}.txt"
        dst_det = f"det_results/mot20_dist/{seq}.txt"
        dst_det_dataset = f"dataset/MOT20_dist/train/{seq}/det/det.txt"
        ...
        # seqinforfile 
        src_seqinfo_file = f"dataset/MOT20/train/{seq}/seqinfo.ini"
        dst_seqinfo_folder = f"dataset/MOT20_dist/train/{seq}/"
        copy_file2dir(src_seqinfo_file, dst_seqinfo_folder)
        
        # Create cammera parameter file
        src_cam_para = f"cam_para/MOT20/{seq}.txt"
        dst_cam_para = f"cam_para/MOT20_dist/{seq}.txt"
        create_cam_para(src_cam_para, dst_cam_para, dist_coeff)
        
        # Create distorted gt 
        src_mot = f"dataset/MOT20/train/{seq}/gt/gt.txt"
        dst_mot = f"dataset/MOT20_dist/train/{seq}/gt/gt.txt"
        create_distorted_mot20(src_mot, dst_mot, dist_coeff, cam_type)
